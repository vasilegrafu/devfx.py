...
def test_pvalue():
    ...
    (n, mu, S, cv, pvalue) = stats.pvalue.mean.normal(data).upper_tailed(mu0, sigma0)
    print((n, mu, S, cv, pvalue))

    figure = dv.Figure((8, 4))

    chart = dv.Chart2d(figure, position=(1, 1, 1))
    stats.normal(0, 1).pdf_on_chart(chart).plot(olNx=2, color='green')
    ...
    # chart = dv.chart2d(figure, position=(1, 1, 1))
    #
    # stats.normal(mu0, sigma0).pdf_on_chart(chart).plot(olNx=1, color='green')
    # chart.axvline(mu0, color='green')
    #
    # stats.normal(mu, sigma).pdf_on_chart(chart).plot(olNx=1, color='red')
