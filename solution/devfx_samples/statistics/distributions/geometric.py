...
def test_GeometricK1Distribution():
    ...
    for p in p_range:
        stats.geometrick1(p).cdf_on_chart(chart).plot()

    chart = dv.Chart2d(figure=figure, position=figure[1,0])
    for p in p_range:
        stats.geometrick1(p).pmf_on_chart(chart).plot()
...
def test_GeometricK0Distribution():
    ...
    chart = dv.Chart2d(figure=figure, position=figure[0,0])
    for p in p_range:
        stats.geometrick0(p).cdf_on_chart(chart).plot()

    chart = dv.Chart2d(figure=figure, position=figure[1,0])
    for p in p_range:
        stats.geometrick0(p).pmf_on_chart(chart).plot()
