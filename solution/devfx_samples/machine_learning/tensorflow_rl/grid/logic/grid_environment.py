...
class GridEnvironment(ml.rl.Environment):
    ...
    """------------------------------------------------------------------------------------------------
    """
    def __enumerate_non_blocked_states(self):
        return [state for state in self.__grid.flat if(state.kind != GridCellStateKind.BLOCKED)]

    def __enumerate_blocked_states(self):
        return [state for state in self.__grid.flat if(state.kind == GridCellStateKind.BLOCKED)]

    def _get_random_state(self):
        states = self.__enumerate_non_blocked_states()
        state = states[np.random.choice(len(states), size=1)[0]]
        return state

    def _get_random_non_terminal_state(self):
        states = [state for state in self.__enumerate_non_blocked_states() if(state.kind == GridCellStateKind.NON_TERMINAL)]
        state = states[np.random.choice(len(states), size=1)[0]]
        return state

    def _get_random_terminal_state(self):
        states = [state for state in self.__enumerate_non_blocked_states() if(state.kind == GridCellStateKind.TERMINAL)]
        state = states[np.random.choice(len(states), size=1)[0]]
        return state
    ...
    def __enumerate_possible_actions(self, state):
        ...
        if((ix[1] > 1) and (self.__grid[ix[0], ix[1]-1].kind != GridCellStateKind.BLOCKED)):
