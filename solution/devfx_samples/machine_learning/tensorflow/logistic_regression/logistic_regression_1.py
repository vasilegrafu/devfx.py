...
"""------------------------------------------------------------------------------------------------
"""
class LogisticRegression1Model(ml.Model):
    # ----------------------------------------------------------------
    ...
    def J(self, x, y):
        hr = self.h(x)
        r = -ml.reduce_mean(ml.cast_to_float32(y)*ml.log(hr)+ml.cast_to_float32(1-y)*ml.log(1.0-hr))
        return r
    ...
    def y_pred(self, x):
        hr = self.h(x)
        r = ml.iverson(ml.greater_equal(hr, 0.5))
        return r
