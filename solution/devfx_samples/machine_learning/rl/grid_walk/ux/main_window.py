...
class MainWindow(ux.Window):
    ...
    def __init_widgets(self):
        ...
        def agent_randomness_combobox__OnItemSelected(sender, event_args): 
            self.agent_randomness_spinbox.SetValue(self.viewable_grid_environment.get_agent(int(self.agent_randomness_combobox.GetValue().split('|')[0])).get_randomness())
            self.grid_canvas.UpdateDrawing()
        self.agent_randomness_combobox.OnItemSelected += agent_randomness_combobox__OnItemSelected
        self.agent_randomness_spinbox = ux.FloatSpinBox(self, min=0.0, max=1.0, initial=0.0, inc=0.05, size=(64, -1))
    ...
    def __grid_canvas__OnDraw(self, sender, event_args):
        ...
        # draw rewards
        for (cell_index, cell_content) in self.viewable_grid_environment.get_walkable_cells().items():
        ...
        # draw policy
        agent = self.viewable_grid_environment.get_agent(id=int(self.agent_randomness_combobox.GetValue().split('|')[0]))
        policy = agent.get_policy()
        for state in policy.qtable:
            for action in policy.qtable[state]:
