...
class LogisticRegression2Model(cg.Model):
    # ----------------------------------------------------------------
    @cg.output_as_tensor((cg.float32, (None, 2)))
    ...
    def h(self, x):
        ...
        r = cg.stack([
            cg.exp(w0 + w1*x)/(1.0 + cg.exp(w0 + w1*x)),
                          1.0/(1.0 + cg.exp(w0 + w1*x))
        ], axis=1)
        return r
    ...
    def J(self, x, y):
        hr = self.h(x)
        r = -cg.reduce_mean(cg.reduce_sum(cg.one_hot(indices=y, depth=2, on_value=1.0, off_value=0.0, axis=1)*cg.log(hr), axis=1))
        return r
