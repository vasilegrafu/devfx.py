...
def test_cdf():
    ...
    rate = 2.0
    chart = dv.Chart2d(figure, position=figure.grid[0,0])
    stats.distributions.poisson(rate).cdf_on_chart(chart).plot(ll=0, ul=40)

    rate = 4.0
    chart = dv.Chart2d(figure, position=figure.grid[1,0])
    stats.distributions.poisson(rate).cdf_on_chart(chart).plot(ll=0, ul=40)

    rate = 8.0
    chart = dv.Chart2d(figure, position=figure.grid[2,0])
    stats.distributions.poisson(rate).cdf_on_chart(chart).plot(ll=0, ul=40)

    rate = 16.0
    chart = dv.Chart2d(figure, position=figure.grid[3,0])
    stats.distributions.poisson(rate).cdf_on_chart(chart).plot(ll=0, ul=40)
